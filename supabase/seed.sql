...
--
--  IDEMPOTENTE: cada bloque solo inserta si la clave `seeded:<tabla>` NO
--  existe todavía en site_content (el mismo marcador que usa la app). Sin
--  esta guarda, `gifts`/`services`/`faqs` no tienen ninguna columna única
--  además de su `id` (uuid aleatorio), así que `on conflict do nothing`
--  NUNCA encontraría un conflicto real: ejecutar el archivo dos veces
--  duplicaría en silencio los 6 regalos + 6 preguntas + 3 servicios de
--  ejemplo. Ejecutar este script las veces que quieras es seguro.
-- ═══════════════════════════════════════════════════════════════════

do $$
begin
  if not exists (select 1 from public.site_content where key = 'seeded:gifts') then
    ...
    insert into public.site_content (key, value) values ('seeded:gifts', jsonb_build_object('at', now()));
  end if;
end $$;

do $$
begin
  if not exists (select 1 from public.site_content where key = 'seeded:faqs') then
    ...
    insert into public.site_content (key, value) values ('seeded:faqs', jsonb_build_object('at', now()));
  end if;
end $$;

do $$
begin
  if not exists (select 1 from public.site_content where key = 'seeded:services') then
    ...
    insert into public.site_content (key, value) values ('seeded:services', jsonb_build_object('at', now()));
  end if;
end $$;
